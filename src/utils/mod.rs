use std::{fs, env, error::Error, io::Read};
...
fn get_web_content_text(url: &str)->Result<String, Box<dyn Error>>{
    let name = *(url
        .split("/")
        .collect::<Vec<&str>>().last().unwrap_or(&"Unknown"));
    println!("Downloading content from {}", style(name).bold());
...
pub fn get_web_content_bytes(url: &str)->Result<Vec<u8>, Box<dyn Error>>{
    let name = url
                .split("/")
                .collect::<Vec<&str>>().last().unwrap_or(&"Unknown").to_owned();
    println!("Downloading Bytes from {}", style(name).bold());
    let mut c = vec![];
