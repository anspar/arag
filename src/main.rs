...
mod types;
mod utils;
use utils::{helpers, yaml_parser, cli};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let conf = match yaml_parser::get_conf(&format!("{}/arag.yml",r_path.display())){
        Ok(v)=>v,
        Err(_)=>{println!("No arag.yml found, using defaults"); AragConf::defaults()}
    };

    let entry_dir = match opt.entry {
        Some(v) => format!("{}/{}", &r_path.display().to_string(), v),
        None => format!("{}/{}", &r_path.display().to_string(),constants::TEMPLATE_DIR)
    };
    ...
    if opt.pkg{
        let p = pkg(&mut hb, &r_path.display().to_string(), &entry_dir, &conf.ipfs_gateway);
    ...
    if opt.show{
        ...
        start_dir_watcher(&mut hb,&r_path.display().to_string(), &entry_dir, &conf.ipfs_gateway);
        return Ok(())        
    }
